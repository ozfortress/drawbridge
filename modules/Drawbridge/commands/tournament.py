    ...
    async def _generate_match(self, match: Citadel.Citadel.Match):
        ...
        if match.away_team is None:
            ...
            overrides = {
                ...
                self.guild.get_role(team_home[3]): discord.PermissionOverwrite(view_channel=True),
                self.guild.get_role(team_away[3]): discord.PermissionOverwrite(view_channel=True),
            }
            all_access = checks._get_role_ids('HEAD', 'ADMIN', 'TRIAL', 'DEVELOPER', 'APPROVED', 'BOT', 'STAFF')
            for role in all_access:
                overrides[self.guild.get_role(role)] = discord.PermissionOverwrite(view_channel=True)
            cat = self.guild.get_channel(category_id)
            if cat == None:
